...
class Graph:
    ...
    def execute_task_call(self, sub_task, graph, network: Network, start_vertex, params: list[Parameter], organizeId):
                ...
                for level_target, level_target_detail in level_target_map.items():
                    try:
                        ...
                        self.logger.log("system",
                                        f"level_target_detail parse error: {level_target} {level_target_detail}")
        ...
        try:
            task_id = self.task_manager.add_task(sub_task, network.get_vertex(start_vertex))
            self.task_manager.cur_execution = [task_id]
            vertexes = network.get_vertexes()
    ...
    def execute_task_plan(self, sub_task, graph, network: Network, start_vertex, params: list[Parameter],
        ...
        try:
            ...
            next_tasks = next_tasks or route.search(task_vertex, self.task_manager)
            next_tasks_list = {}
            ...
            if self.trace.level > 0:
                ...
                raise Exception(f"level error: {repr(self.trace)}")

            # 获取当前需要执行的任务
            task_vertexes, third_party_task_vertexes = self.task_manager.get_cur_execution_tasks()

            # 向 trace 中添加节点
            adding_vertexes = [n.name for n in task_vertexes]
            adding_vertexes.extend([n.name for n in third_party_task_vertexes])
            ...
            for task in task_vertexes:
                try:
                    ...
                    self.execution_history.append(
                        History(pre_executors=self.task_manager.get_tasks(task.get_prev()), cur_executor=task))
                    self.cur_execution = self.execution_history[-1]
                    ...
                    # 把下一组任务注册到任务管理器中
                    if len(next_tasks_list) > 0:
                        self.task_manager.add_next_tasks(task, next_tasks_list.get("id"), next_tasks_list.get("task"),
                                                         next_tasks_list.get("executor"))
    ...
    def summarize_result(self,
        ...
        messages = self.vertex_messages[task_vertex.name]
        len_message = len(messages)
        ...
        cur_execution_result, next_executables = network.execute(task_vertex.name, messages, **valuable_context)
