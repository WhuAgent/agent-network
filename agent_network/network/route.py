...
class Route(Communicate):
    ...
    def user_task_complete(self, user_task, history_task_info, current_context):
        messages = [
            SystemMessage(
                "你是一个专业的任务进程管理者，擅长判断任务是否已经完成。现在给定用户需求、历史执行任务和当前执行上下文，判断任务是否已经完成。请以 json 形式返回，包括一个字段 complete，值为 true 或 false。"),
            UserMessage(
                f"现在用户的需求为{user_task}, 历史执行任务为{history_task_info}，当前执行上下文为{current_context}")
        ]
    ...
    def plan_based_soft_route(self, executor, vertexes_description, user_task, history_task_info, valuable_context,
                              next_tasks):
        agent_description = {}
        ...
        if len(agent_description) > 0:
            messages = [
                ...
                UserMessage(
                    f"现在用户的需求是{user_task}, 在执行用户需求时，历史执行任务为{history_task_info}，当前执行上下文为{valuable_context}，根据用户需求、历史执行任务和当前执行上下文信息，是否可以直接执行预期的下一步任务{next_tasks}？如果不行，请重新对下一步任务进行规划")
            ]
